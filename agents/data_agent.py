...
def data_dict_to_text(dataframe_dict):
    res = []
    for name, df in dataframe_dict.items():
        res.append(f'DataFrame name : {name}')
        res.append('sample data:')
        res.append(f'{df.head(5).to_markdown()}')

    res = '\n'.append(res)
    return res

class DataAgent:
    def __init__(self, llm: str, dataframe_dict: list):
        self.llm = llm
    ...
    def construct_prompt(self, question: str) -> str:
        ...
        return f"""
        SYSTEM:
        You are a data assistant. Given below is the list of dataframes available to you  that are already loaded in python.
        {data_dict_to_text(self.dataframe_dict)}
        
        Convert the user query into Python code. Return only the code.
        USER:
        {question}
        """
    def construct_prompt_with_metadata(self, question: str, metadata:str) -> str:
